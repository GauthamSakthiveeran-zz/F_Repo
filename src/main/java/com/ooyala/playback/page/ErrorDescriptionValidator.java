...
public class ErrorDescriptionValidator extends PlayBackPage implements PlaybackValidator {
    ...
    public boolean validate(String element, int timeout) throws Exception {
        ...
        extentTest.log(LogStatus.PASS,"Error code :"+errorCode +"and error description verified successfully");
        logger.info("Error code :"+errorCode +"and error description verified successfully");
        return true;
    }
    ...
    private boolean errorDescription() {
        ...
        if (!text.contains(expectedErrorDesc)) {
            extentTest.log(LogStatus.FAIL, "Error message is wrong");
            logger.error("Error message is wrong");
            return false;
        }

        logger.info("Error description is present");
        extentTest.log(LogStatus.PASS,"Error description is present");
        return true;
    }
    ...
    public ErrorDescriptionValidator validateErrorDescription(String language) {
        try{
            FileReader reader=new FileReader("src/main/resources/languages.properties");
            Properties p=new Properties();
            p.load(reader);
            expectedErrorDesc = p.getProperty(language);
        }catch (Exception ex){
