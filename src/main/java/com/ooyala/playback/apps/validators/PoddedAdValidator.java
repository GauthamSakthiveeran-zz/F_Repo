...
public class PoddedAdValidator extends PlaybackApps implements Validators {
    ...
    public boolean validate(String element, int timeout) throws Exception {
        ...
        result = result && notificationEventValidator.verifyEvent(Events.PLAYBACK_STARTED, 70000);
        ...
        if(isOoyalaSkinSampleApp)
        {
            ...
            result = result && notificationEventValidator.validateEvent(Events.SEEK_STARTED, 70000);
            result = result && notificationEventValidator.validateEvent(Events.SEEK_COMPLETED, 70000);
        ...
        else
        {
            ...
            if(getPlatform().equalsIgnoreCase("android")){
                result = result && notificationEventValidator.verifyEvent(Events.PLAYBACK_PAUSED_ANDRD, 70000);
            }
            else{
                result = result && notificationEventValidator.verifyEvent(Events.PLAYBACK_PAUSED, 70000);
            }
            ...
            result = result && notificationEventValidator.verifyEvent(Events.SEEK_STARTED, 70000);
            result = result && notificationEventValidator.verifyEvent(Events.SEEK_COMPLETED, 70000);
            result = result && iOS ? pauseAction.startAction(iosPlayPause)
                    : pauseAction.startAction("PLAY_PAUSE_ANDROID");
            if(getPlatform().equalsIgnoreCase("android")){
                result = result && notificationEventValidator.verifyEvent(Events.PLAYBACK_RESUMED_ANDRD, 70000);
