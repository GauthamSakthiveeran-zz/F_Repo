...
public class OoyalaSkinSampleSkinPlayBackLockUnlockTest extends OoyalaSkinSampleAppSkinPlaybackUtils {

	private static Logger logger = Logger.getLogger(OoyalaSkinSampleSkinPlayBackLockUnlockTest.class);
	...
	public void testOoyalaSkinPlaybackApp(String testName, TestParameters test) throws Exception {
		...
		try {
			...
			result = result && androidKeyCode.lockTheDevice();
			result = result && notificationEventValidator.validateEvent(Events.PLAYBACK_SUSPENDED, 70000);
			Thread.sleep(2000);
			result = result && androidKeyCode.startAction("UNLOCK");
