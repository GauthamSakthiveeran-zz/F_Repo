...
//            }
    ...
    public boolean verifyDiscoveryTextShown(String ccTextShown) {
        int count;
        //First to event of CC are always empty thats why validating from the 3rd event of CC
        for (count = 3; count <= 7; count++) {
            String ccTextElement = ccTextShown + count;
            //Find the element ccShowingText_count and get its text
            WebElement element1 = driver.findElement(By.id(ccTextElement));
            String element1Text = element1.getText();
            //incrementing the value of count variable to get the next cc event
            count++;
            ccTextElement = ccTextShown + count;
            //Find the element ccShowingText_count and get its text
            WebElement element2 = driver.findElement(By.id(ccTextElement));
            String element2Text = element2.getText();
            //checking whether element1Text is empty
            if (!element1Text.isEmpty()) {
                //checking whether element2Text contains substring element1Text
                if (element2Text.contains(element1Text)) {
                    logger.error("CC's are not cleared and keep piling up");
                    logger.info("Previous CC text :" + element1Text);
                    logger.info("Current CC text :" + element2Text);
                    extentTest.log(LogStatus.FAIL, "CC's are not cleared and keep piling up");
                    return false;
                }
            }
        }
        logger.error("CC's are cleared and not piling up");
