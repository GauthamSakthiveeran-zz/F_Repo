...
public class PlaybackMidrollOverlayTests extends PlaybackWebTest {
	...
	public void verifyMidrollOverlay(String testName, UrlObject url) throws OoyalaException {
		...
		try {
			...
			result = result && midrollAdValidator.isOverlayPlayTimePresent(url)
					? midrollAdValidator.setTime(url.getOverlayPlayTime()).validateAdStartTime("showNonlinearAd_1")
					: event.validate("showNonlinearAd_1", 160000);

			result = result && adClicks.overlay().validate("", 120000);
			result = result && getBrowser().equalsIgnoreCase("internet explorer") ? seek.seek(10, true)
					: seekValidator.validate("seeked_1", 10000);
