...
public class PlayBackAsserts {
	...
	/**
	 * failures are a list of failures - used for soft asserts. ExtentTest is for reporting. The class needs to be instantiated at @BeforeMethod in PlaybackWebTest
	 * @param failures
	 * @param extentTest
	 */
	...
	public PlayBackAsserts equals(String actual, String expected, String stepMessage) { // can add a similar notEquals when needed.

		if (actual == null || expected == null) {
			failures.add(stepMessage + " Actual:" + actual + " Expected:" + expected);
			extentTest.log(LogStatus.FAIL, stepMessage);
			return this;
		} else if (!actual.equals(expected)) {
			String msg = stepMessage + " Actual:" + actual + " not equals " + " Expected:" + expected;
			failures.add(msg);
			extentTest.log(LogStatus.FAIL, stepMessage);
		} else{
