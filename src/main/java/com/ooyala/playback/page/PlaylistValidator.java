...
public class PlaylistValidator extends PlayBackPage implements PlaybackValidator {
    ...
    public boolean validate(String element, int timeout) {
        ...
        /*    getWebElement("PREVIOUS_ARROW").click();
            ...
            extentTest.log(LogStatus.FAIL, "Element is not found" + noSuchElement.getMessage());
            return false;
        } catch (Exception e) {
            logger.error("Error while validating playlist " + e.getMessage());
            extentTest.log(LogStatus.FAIL, "Error while validating playlist " + e.getMessage());
            return false;
        }
    }
    ...
    */
        ...
        for (int i = 0; i < 3; i++) {
            try {
                ...
                if (driver.findElement(By.id(asset)).isDisplayed()) {
                    ...
                    if (!result) {
                        logger.error("Failed to play video : " + count);
        ...
        try {
            if (getBrowser().equalsIgnoreCase("MicrosoftEdge")) {
                try {
                    ...
                    logger.error("Error while focus on element play button." + e.getMessage());
                    e.printStackTrace();
                }
            }
            ...
            if (!waitOnElement(By.id("playing_" + count), 10000)) {
                if (!factory.getPlayValidator().validate("playing_" + count + "", 20000)) {
                    ...
                    extentTest.log(LogStatus.FAIL, "error while playing");
                    return false;
                }
            } else {
            ...
            if (getBrowser().equalsIgnoreCase("internet explorer")) {
                ...
                if (!factory.getEventValidator().validate("seeked_" + count, 20000)) {
                    logger.error("error after seeking the video");
                    extentTest.log(LogStatus.FAIL, "error after seeking the video");
                    return false;
                }
            } else {
    ...
    public boolean getOrientation(String orientationValue) {
        ...
        if (!orientation.contains(orientationValue)) {
            ...
            logger.error("Orientation is incorrect");
            return false;
        }
    ...
    public boolean getPosition(String positionValue) {
        ...
        if (!position.contains(positionValue)) {
            ...
            logger.error("Playlist Position is incorrect");
            return false;
        }
        return selectAssetFromPlaylist();
    }
                    ...
                    if (isPagingElementActive) {
                        ...
                        for (int j = 0; j < assetsUnderPagingElement - 2; j++) {
                            ...
                            if (!result) {
                                ...
                                logger.error("Issue with playback of video no " + count);
                            }
                            //j = j + 2;
                            count++;
                        }
    ...
    public boolean getFirstVideoFromPlaylist(String value) {
        ...
        if (value.equalsIgnoreCase("true")) {
            if (!emebedCodeOfFirstAsset.contains(emebedCodeOfCurrentAsset)) {
                logger.error("emebedCodeOfFirstAsset should be the same as emebedCodeOfCurrentAsset");
        ...
        if (value.equalsIgnoreCase("false")) {
            if (emebedCodeOfFirstAsset.contains(emebedCodeOfCurrentAsset)) {
                logger.error("emebedCodeOfFirstAsset should not be the same as emebedCodeOfCurrentAsset");
    ...
    public boolean getThumbnailSize(String thumbnailSizeValue) {
        ...
        if (!thumbnailSize.equals(thumbnailSizeValue)) {
            logger.error(" Playlist Caption Position is incorrect. Expected : " + thumbnailSize);
    ...
    public boolean getThumbnailSpacing(String thumbnailSpaceValue) {
        ...
        for (int i = 0; i < numberOfAsset; i++) {
            ...
            if (obtainedThumbnailSize != givenThumbnailSize) {
                logger.error(" Issue with Thumbnail spacing.");
    ...
    public boolean getMenuSytle(String value) {
        if (!waitOnElement("PLAYLISTS_PLAYER", 10000)) {
            logger.error("wait on element PLAYLISTS_PLAYER failed");
            extentTest.log(LogStatus.FAIL, "wait on element PLAYLISTS_PLAYER failed");
            return false;
        }
        ...
        if (totalPlaylists == 1) {
            ...
            logger.error("There is only one playlist");
            return false;
        }

        logger.info("Total no of playlists : " + totalPlaylists);
        ...
        for (int i = 0; i < totalPlaylists; i++) {
            ...
            if (!playlistId.contains(plalistName)) {
                logger.error(playlistId + "does not contain :" + plalistName);
                extentTest.log(LogStatus.FAIL, playlistId + "does not contain :" + plalistName);
                return false;
            }
            ...
            if (!playlist.getAttribute("class").contains("oo-menu-active")) {
                ...
                logger.error("Correct tab not selected.");
            }
        }
        return checkPlayback(1);
    }
    ...
    public boolean scrollToEitherSide() {
        ...
        if (getWebElement("PREVIOUS_ARROW").isDisplayed()) {
            ...
            logger.error("PREVIOUS_ARROW should not be present.");
            flag = false;
        }
        ...
        for (int i = 0; i < totalPlaylistVideo; i++) {
            try {
                ...
                if (getWebElement("SCROLL_DOWN").isDisplayed()) {
                    ...
                    logger.error("SCROLL_DOWN should not be present.");
                    flag = false;
                }
                break;
            }
    ...
    public boolean getWrapperFontSize(String wrapperSizeValue) {
        ...
        if (!fontSize.contains(wrapperSizeValue)) {
            logger.error(fontSize + "does not contain :" + wrapperSizeValue);
            extentTest.log(LogStatus.FAIL, fontSize + "does not contain :" + wrapperSizeValue);
            return false;
        }
        return checkPlayback(1);
    }
