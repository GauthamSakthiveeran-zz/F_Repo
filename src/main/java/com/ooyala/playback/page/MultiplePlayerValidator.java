...
public class MultiplePlayerValidator extends PlayBackPage implements PlaybackValidator  {
    ...
    public boolean validate(String element, int timeout) throws Exception {
            ...
            if (element.equalsIgnoreCase("player1_play")) {
                ...
                if(!clickOnHiddenElement("PLAYER1_START_SCREEN")){ logger.error("PLAYER1_START_SCREEN locator is not present");return false;};
                logger.info("Player 1 playing");
            }

            if (element.contains("player1_pause")) {
                waitOnElement("PLAYER1_PAUSE", timeout);
                if(!clickOnHiddenElement("PLAYER1_PAUSE")){logger.error("PLAYER1_PAUSE locator is not present");return false;};
                logger.info("Player 1 paused");
            }

            if (element.equalsIgnoreCase("player2_play")) {
                waitOnElement("PLAYER2_START_SCREEN", timeout);
                if (!clickOnHiddenElement("PLAYER2_START_SCREEN")){logger.error("PLAYER2_START_SCREEN locator is not present"); return false;};
                logger.info("Player 2 playing");
            }

            if (element.contains("player2_pause")) {
                waitOnElement("PLAYER2_PAUSE", timeout);
                if (!clickOnHiddenElement("PLAYER2_PAUSE")){logger.error("PLAYER2_PAUSE locator is not present"); return false;};
