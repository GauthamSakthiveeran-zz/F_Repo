...
public class EventValidator extends PlayBackPage implements PlaybackValidator {
	...
	public boolean validate(String element, int timeout) throws Exception {
				...
				if (element.toLowerCase().contains("ad") && !element.toLowerCase().contains("nonlinear")) {
					if (element.toLowerCase().contains("willplay")) {
						if (!(waitOnElement(By.id("analytics_ad_break_started_1"), timeout)
							...
							logger.error(
									"analytics_ad_started_1 or analytics_ad_break_started_1 analytics element is not present for ad start");
							extentTest.log(LogStatus.FAIL,
									"analytics_ad_started_1 or analytics_ad_break_started_1 analytics element is not present for ad start");
							return false;
						}
						logger.info(
								"analytics_ad_started_1 and analytics_ad_break_started_1 analytics elements are present for ad start");
					...
					if (element.toLowerCase().contains("played")) {
						if (!(waitOnElement(By.id("analytics_ad_break_ended_1"), timeout)
							...
							logger.error(
									"analytics_ad_ended_1 or analytics_ad_break_ended_1 analytics element is not present for ad start");
							extentTest.log(LogStatus.FAIL,
									"analytics_ad_ended_1 or analytics_ad_break_ended_1 analytics element is not present for ad start");
							return false;
						}
						logger.info(
								"analytics_ad_ended_1 and analytics_ad_break_ended_1 analytics elements are present for ad start");
						extentTest.log(LogStatus.PASS,
								"analytics_ad_ended_1 and analytics_ad_break_ended_1 analytics elements are present for ad start");
					}
				}
		...
		if (waitOnElement(By.id(element), timeout)) {
			 ...
			 * if(getBrowser().equalsIgnoreCase("internet explorer")) return
			 * true; ((JavascriptExecutor)
			 * driver).executeScript("return pp.pause();"); boolean flag =
			 * scrubberValidator.validate("", 1000); ((JavascriptExecutor)
			 * driver).executeScript("return pp.play();"); return flag; }
			 */
			return true;
		}
	...
	public boolean validateVideoElementOccuredCount(int timeout) {
		...
		boolean result = true;
		try {
			int noOfPoddedAds = parseInt((((JavascriptExecutor) driver)
					.executeScript("return document.getElementById('adPodStarted_1').textContent")).toString());
